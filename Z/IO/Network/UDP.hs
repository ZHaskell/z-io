...
instance Show UDPClient where
    show (UDPClient handle slot uvm _ bufsiz _ _ _) =
        "UDPClient{udpHandle = " ++ show handle ++
...
data UDPClientConfig = UDPConfig
    { sendMsgSize :: {-# UNPACK #-} !Int           -- ^ maximum size of sending buffer
    , localUDPAddr  :: Maybe (SocketAddr, UVUDPFlag) -- ^ do we want bind a local address before receiving & sending?
                                                   --   set to Nothing to let OS pick a random one.
    } deriving (Show, Eq, Ord)

-- | default 'UDPConfig', @defaultUDPConfig = UDPConfig 512 6 512 Nothing@
defaultUDPClientConfig = UDPClientConfig 512 Nothing

-- | Initialize a UDP socket.
--
initUDPClient :: HasCallStack
              => UDPConfig
              -> Resource UDPClient
initUDPClient (UDPConfig rbsiz rbArrSiz sbsiz maddr) = initResource
    (do uvm <- getUVManager
        ...
        return (UDPClient handle slot uvm rbuf rbsiz' rbufArr sbuf closed))
    closeUDP
  where
    rbsiz' = max 0 rbsiz
...
closeUDP :: UDPClient -> IO ()
closeUDP (UDPClient handle _ uvm _ _ _ _ closed) = withUVManager_ uvm $ do
    c <- readIORef closed
...
sendUDP (UDPClient handle slot uvm _ closed) addr (V.PrimVector ba s la) = mask_ $ do
    c <- readIORef closed
    when c throwECLOSED
...
setTTL :: UDPClient -> Int -> IO ()

setBroadcast :: UDPClient -> Int -> IO ()


setMulticastTTL :: UDPClient -> Int -> IO ()


setMulticastInterface :: UDPClient -> Int -> IO ()


setMulticastLoop :: UDPClient -> Int -> IO ()
...
getSockName (UDP handle _ _ _ _ _ _ closed) = do
    c <- readIORef closed
    when c throwECLOSED
    withSocketAddrStorage $ \ paddr plen ->
        withPrimUnSafe (fromIntegral sizeOfSocketAddrStorage :: CInt) $ \ plen ->
...
recvUDP :: HasCallStack => UDP -> IO [(Maybe SocketAddr, Bool, V.Bytes)]
